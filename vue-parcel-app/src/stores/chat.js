...
const mockConversation = [
  ...
  // 11. 봇의 냉장고 속 재료 활용하기 답변
  {
    ...
    content: {
        ...
        recommendation: [
            { menu: '1. 구운가지 닭가슴살 샐러드와 들깨 소스', ingredients: '닭가슴살, 가지, 잎채소, 올리브유, 들깨 드레싱, 플레인요거트, 양파, 레몬즙, 꿀, 들깨가루' },
  ...
  // 백엔드에 요청 문구를 전달하면 백엔드에서 API를 활용해 정보 조회하고 프론트엔드로 전달

  // "선택한 메뉴 영양 정보 보기" 버튼이나 문구 입력 : 레시피를 찾았다는 것으로 API1 사용
  // API 1 : https://www.foodsafetykorea.go.kr/api/openApiInfo.do?menu_grp=MENU_GRP31&menu_no=661&show_cnt=10&start_idx=1&svc_no=COOKRCP01#
  // API 1은 요리 레시피 DB를 제공해주고, 레시피마다의 영양 정보도 함께 제공함
  // API 2 : https://www.data.go.kr/data/15100070/standard.do#/tab_layer_grid
  // API 2는 레시피 제공 없이 특정 요리에 대한 영양 정보 제공
  // API 1로 찾아보고 없다면 2에서 정보 추출
